...
func buildConf(privKeyHex, nodeIP, pool6CIDR, pool6Prefix string, peers, allowedSources, ignoredDstSubnets []string) string {
	...
	sb.WriteString("    #   return-ipv6-addresses: true\n")
	sb.WriteString("    # }\n")
	sb.WriteString("    # Example: Blocked (\"empty\") zone — a zone with NEITHER a prefix\n")
	sb.WriteString("    # NOR return flags can produce no answer at all, so it replies with\n")
	sb.WriteString("    # a local NXDOMAIN for every query type in its domains, without\n")
	sb.WriteString("    # contacting any forwarder.\n")
	sb.WriteString("    # {\n")
	sb.WriteString("    #   domains: [\"blocked.example\"]\n")
	sb.WriteString("    # }\n")
	sb.WriteString("  ]\n\n")

	sb.WriteString("  # Local authoritative answers for exact domain names (values are\n")
	sb.WriteString("  # literal IPv4 or IPv6 addresses — A queries get the v4 value as an A\n")
	sb.WriteString("  # record, AAAA queries the v6 value as an AAAA record; no NAT64\n")
	sb.WriteString("  # synthesis). Static names are served before any zone rules, never\n")
	sb.WriteString("  # touch a forwarder, and win over blocked zones. Exact-name matching\n")
	sb.WriteString("  # only. Reloadable via SIGHUP.\n")
	sb.WriteString("  # Dns64Static: {\n")
	sb.WriteString("  #   \"router.home\": \"192.168.1.1\"\n")
	sb.WriteString("  #   \"nas.home\": \"2001:db8::10\"\n")
	sb.WriteString("  # }\n\n")
